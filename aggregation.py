...
class LSTMAggregator(nn.Module):
    ...
    def forward(self, gene_features: dict, device):
        """
        :param device: Hardware device execution target
        :param gene_features: key: node idx, value: tensor set of image features
        :return: Collated global node tensor feature layout map
        """
        n_data_feature = []
        ...
        for i in node_seq:
            ...
            # Randomly shuffle the instances order to force permutation invariance
            r = torch.randperm(feature.size(0))
            _, (h, _) = self.aggregator(feature[r].unsqueeze(0))
            n_data_feature.append(h.squeeze(0))
            
        n_data_feature = torch.cat(n_data_feature, dim=0)
